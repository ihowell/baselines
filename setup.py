...
extras = {
    'test': ['filelock', 'pytest', 'pytest-forked', 'atari-py~=0.2.0'],
    'bullet': ['pybullet'],
    'mpi': ['mpi4py']
}
...
setup(
    ...
    install_requires=[
        'gym>=0.15.4, <0.16.0', 'scipy', 'tqdm', 'joblib', 'dill',
        'progressbar2', 'cloudpickle', 'click', 'opencv-python'
    ],
...
assert LooseVersion(re.sub(r'-?rc\d+$', '',
                           tf_pkg.version)) >= LooseVersion('2.0.0')
